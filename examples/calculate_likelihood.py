...
gp_loglike = gpOBJ.log_likelihood(kernels.Matern32(10, 1) + kernels.WhiteNoise(0.1))
print(f"GP log-likelihood = {gp_loglike}")
...
# Trying with a new kernels
tp_loglike = tpOBJ.log_likelihood(kernels.Matern32(10, 1) + kernels.WhiteNoise(0.1), 5)
print(f"TP log-likelihood = {tp_loglike}")
assert tp_loglike == -61.26479588055934

tp_loglike = tpOBJ.log_likelihood(
    kernels.PiecewiseSE(1, 2, 10) + kernels.WhiteNoise(0.1), 5
)
print(f"TP log-likelihood = {tp_loglike}")
assert tp_loglike == -101.83522277011839
